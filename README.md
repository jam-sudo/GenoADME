...
Status: pre-release (v0.3 closed via PATH 3). Tier 1 validation re-run 2026-05-09 under Sisyphus pin `bf764c5` (v0.3.3 / PR #33) — **result: PASS**. Current canonical: [`reports/validation-tier1-20260509.md`](reports/validation-tier1-20260509.md) / [`reports/headline-metrics-20260509.json`](reports/headline-metrics-20260509.json). All three criteria pass: Population AAFE (AUC) 1.825 ≤ 2.0; PM/EM AUC ratio **3.574 ∈ [1.4, 5.0]** (band re-spec'd 2026-05-09 under `tier-change:` correcting an identified citation error in the original v0.1.0 band); PM/EM Cmax ratio 2.579 ≥ 1.3. The PM/EM AUC ratio (3.574) is inside Niemi 2006 men-stratum 95% CI [1.74, 4.91] — the model is empirically consistent with primary clinical data. The prior v0.3 closing (2026-05-08, override-based, commit `a13d2d9`) is superseded by PATH 3 (2026-05-09, no override) — see [`docs/v0.3-meta-analysis.md` §2.10](docs/v0.3-meta-analysis.md), [`docs/tier-changes.md`](docs/tier-changes.md), and [`docs/limitations.md` §11](docs/limitations.md) for the ultrathink-driven correction record. All six Tier 1 runs (2026-04-29, 2026-05-01, 2026-05-03, 2026-05-08-baseline, 2026-05-08-override, 2026-05-09-PATH-3) are part of the audit chain ([`reports/audit-log.jsonl`](reports/audit-log.jsonl)) with `worktree_clean` + `deps.sisyphus` fields per the [`audit.log_query` gate](src/genoadme/audit.py) (GenoADME issue #1).
...
|Milestone                                       |GenoADME                                                                 |Sisyphus dependency                                                            |Status / Closing condition                                                                                                                                                                                                                                                                                                                                                                                          |
|------------------------------------------------|-------------------------------------------------------------------------|-------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|**v0.2 — Tier 1 calibration alignment** ✅ closed|[milestone/1](https://github.com/jam-sudo/GenoADME/milestone/1)          |[Sisyphus #8](https://github.com/jam-sudo/Sisyphus/issues/8) closed 2026-05-02 |Closed 2026-05-03 with PARTIAL outcome by design. Sisyphus pin bumped to issue #8 closing state (`9f1680d`); Tier 1 re-anchored per `tier-change:` commit `f96b5fc` (FDA Cmax + Niemi ratio); AAFE PASS confirmed under FDA anchor. PM/EM AUC ratio over-shoot carved out as v0.3.                                                                                                                                  |
|**v0.3 — CPIC PM=0.10× scale investigation** ✅ closed (PATH 3) |[milestone/3](https://github.com/jam-sudo/GenoADME/milestone/3) |[Sisyphus #31](https://github.com/jam-sudo/Sisyphus/issues/31) closed by PR #33 (v0.3.3, 2026-05-08) |**Re-closed 2026-05-09** via PATH 3 (citation error correction). Initial closing (2026-05-08, P-ii override-based, commit `a13d2d9`) was superseded after ultrathink self-audit identified that the original v0.1.0 band [1.4, 2.5] was based on a flawed citation chain (Pasanen 2007 is on atorvastatin / rosuvastatin, NOT pravastatin), and the model's no-override output (PM/EM AUC ratio 3.574) is INSIDE Niemi 2006 men-stratum 95% CI [1.74, 4.91]. PATH 3: `tier-change:` band [1.4, 2.5] → [1.4, 5.0] (commit `57e9c8b`) + override revert. v0.3 closes via `metric-change:` commit `3f93c16` with no override applied. Sparse-evidence caveat preserved in [`docs/v0.3-meta-analysis.md`](docs/v0.3-meta-analysis.md) §2.5/§2.10. |
|**v0.4 — Activate Deferred pairs**              |[milestone/2](https://github.com/jam-sudo/GenoADME/milestone/2)          |Sisyphus PR #32 partially unblocks NAT2 + UGT1A1; Sisyphus #11 still gates prodrug pairs |Promote at least one pair from [`docs/validation-tiers.md`](docs/validation-tiers.md) §Deferred back to an active tier under a `tier-change:` commit per [`docs/commit-discipline.md`](docs/commit-discipline.md) §4. NAT2/isoniazid is the most likely first candidate — physiology-side prerequisites resolved by Sisyphus PR #32 (v0.3.2), remaining work is GenoADME-side data integration. UGT1A1 phenotype-side resolved similarly; irinotecan→SN-38 still gated on Sisyphus #11. CYP2C19/clopidogrel + CYP2C9/warfarin + SLCO1B1/simvastatin still gated on Sisyphus #11. See [`docs/limitations.md` §9.4](docs/limitations.md). |

Milestones intentionally separated **calibration** (v0.2 + v0.3, both closed) from **capability** (v0.4 — extends the supported set of drugs/genes). v0.3's per-substrate calibration mechanism (`phenotype_scale_overrides`) is now the methodology foundation any v0.4 pair will inherit.
