...
Status: pre-release (v0.1.0). Tier 1 validation re-run 2026-05-08 under Sisyphus pin `bf764c5` (v0.3.3 / PR #33 merge — includes PR #32 NAT2/UGT1A1 propagation + back-solve cancellation fix, and the new `phenotype_scale_overrides` API hook closing Sisyphus #31) — **baseline result (no override): PARTIAL**. The 2026-05-08 run [supersedes](reports/validation-tier1-20260503.md) the 2026-05-03 numbers. Current canonical baseline: [`reports/validation-tier1-20260508.md`](reports/validation-tier1-20260508.md) / [`reports/headline-metrics-20260508.json`](reports/headline-metrics-20260508.json). Population AAFE (AUC) **passes** (1.825 ≤ 2.0; tighter margin than under prior pin because PR #32 corrected the back-solve cancellation that had been silently inflating ratios). PM/EM AUC ratio dropped from 4.482 → 3.574 purely from PR #32 — still FAIL band [1.4, 2.5] but the residual is the structural CPIC PM=0.10× compounding effect documented in [`docs/limitations.md` §11](docs/limitations.md). The v0.3 P-ii calibration commit (next) will apply the new `phenotype_scale_overrides` hook to bring PM/EM AUC ratio toward Niemi 2006 men-stratum 95% CI lower bound per [`docs/v0.3-meta-analysis.md`](docs/v0.3-meta-analysis.md). All four Tier 1 runs are part of the audit chain ([`reports/audit-log.jsonl`](reports/audit-log.jsonl)); the 2026-05-08 entry includes the new `worktree_clean` + `deps.sisyphus` fields per the [`audit.log_query` gate](src/genoadme/audit.py) (GenoADME issue #1).
...
|**v0.3 — CPIC PM=0.10× scale investigation**    |[milestone/3](https://github.com/jam-sudo/GenoADME/milestone/3)          |None — investigation lives in GenoADME (eQTL/study meta-analysis layer)        |Resolve PM/EM AUC ratio over-shoot (4.482 vs band [1.4, 2.5]) via either (a) per-substrate CPIC scale calibration with study meta-analysis, or (b) `tier-change:` band re-spec backed by empirical evidence. Closes when Tier 1 re-run produces all three criteria PASS via a `metric-change:` commit per [`docs/commit-discipline.md`](docs/commit-discipline.md) §3. Mechanism: [`docs/limitations.md` §11](docs/limitations.md). |
|**v0.4 — Activate Deferred pairs**              |[milestone/2](https://github.com/jam-sudo/GenoADME/milestone/2)          |[Sisyphus milestone/2](https://github.com/jam-sudo/Sisyphus/milestone/2) (issues [#9](https://github.com/jam-sudo/Sisyphus/issues/9), [#10](https://github.com/jam-sudo/Sisyphus/issues/10), [#11](https://github.com/jam-sudo/Sisyphus/issues/11)) |Promote at least one pair from [`docs/validation-tiers.md`](docs/validation-tiers.md) §Deferred back to an active tier under a `tier-change:` commit per [`docs/commit-discipline.md`](docs/commit-discipline.md) §4. Renumbered from v0.3 on 2026-05-03 — the v0.3 slot now holds the CPIC PM scale investigation, which should land before pair expansion so new pairs inherit the post-investigation PM/EM band.                |

Milestones intentionally separate **calibration** (v0.2 closed; v0.3 — moves the v0.2 PARTIAL number) from **capability** (v0.4 — extends Sisyphus to support drugs / genes the current build cannot represent at all). v0.3 and v0.4 can be picked up independently in principle, but v0.3 first is preferable so v0.4 inherits the post-investigation band.
